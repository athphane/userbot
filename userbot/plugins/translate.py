...
async def translate(_, message: Message):
    if not YANDEX_API_KEY:
        await message.edit(
            "NO API KEY found in configuration. Get one from https://tech.yandex.com/translate/",
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(
            f"https://translate.yandex.net/api/v1.5/tr.json/translate?key={YANDEX_API_KEY}&text={txt}&lang=en"
        ) as resp:
